...
- Enabling [nested layouts](/guide/routing/pages-and-layouts) in the same route segment level:
...
To create multiple [root layouts](/guide/routing/pages-and-layouts#root-layout-required), remove the top-level `layout.js` file, and add a `layout.js` file inside each route groups. This is useful for partitioning an application into sections that have a completely different UI or experience.
